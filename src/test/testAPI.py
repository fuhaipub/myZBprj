...
def url_get_market():
    ...
    '''
    url_http_path = "/data/v1/markets"

    headers = {"Content-type": "text/javascript;charset=utf-8",
               "Accept": "text/plain"}
    conn = httplib.HTTPConnection(url_API_hostname)
    conn.request('GET', url_http_path ,  headers=headers)
    httpRes = conn.getresponse()

    res = {}
    res = json.loads( httpRes.read(), encoding="utf-8" )

    return (httpRes.status, httpRes.reason, res)




def url_get_ticker(marketType = "eos_usdt"):
    """
    返回一种币种的行情
    ...
    vol : 成交量(最近的24小时)
    """
    url_http_path = "/data/v1/ticker" + "?market=" + marketType
...
def url_get_depth(marketType = "eos_usdt", size= 50):
    """
    ...
    (200, 'OK', {u'error': u'\u5e02\u573a\u9519\u8bef'})
    """
    url_http_path = "/data/v1/depth"+ "?"+ "market="+marketType+"&"+"size=" + str(size)
...
def url_get_trades(marketType = "eos_usdt"):
    """
    历史成交
    ...
    (200, 'OK', {u'error': u'\u5e02\u573a\u9519\u8bef'})
    """
    url_http_path = "/data/v1/trades"+ "?"+ "market=" + marketType  # +"&"+"since=" + str(size)
...
if __name__ == '__main__':
    print url_get_market()
    print url_get_ticker("eos_usdt")
    print url_get_depth("eos_usdt",3)
    print url_get_trades("eos_usdt")